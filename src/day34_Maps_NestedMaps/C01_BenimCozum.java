...
public class C01_BenimCozum {
    public static void main(String[] args) {
        ...
        System.out.println(ogrenciMap.get(102).get("Sinif")); // 10
         ...
         //103 numarali ogrenci sinifi 12 yapalim

        String ogrencim=ogrenciMap.get(102).get("Sinif");

        Map<String, String> stringStringMap = ogrenciMap.get(101);
        String ogrenciIsim = stringStringMap.get("isim");
        for (Map.Entry<String,String> eachOgrenci:ogrenciMap.get("isim").entrySet()
             ) {
                    String gelen=eachOgrenci.getValue();
                    if (gelen.equalsIgnoreCase("12")){
                        eachOgrenci.setValue("12");
                    }
        }
